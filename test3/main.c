...
void main(){
	...
	afficherbg(&mask, screen);
        ...
        // Collision detection
        if (CollisionParfaite(mask.img, p1.pos)) {
            // Handle collision
            // For example, prevent player movement or decrease health
        }

        // Update tempsDebut with current ticks before displaying the timer
