...
void UpdateMinimap(Minimap *minimap, SDL_Rect playerAbsolutePosition, int screenWidth, int levelWidth) {
    // Calculate the scaling factor for the minimap based on the screen width and level width
    float scale = (float)minimap->miniaturePosition.w / levelWidth;

    // Adjust the scaling factor to make the representation move slower
    float adjustedScale = scale * 1.2; // Adjust the factor (0.8 here) for slower movement
    /////////
     int playerYOnMap = (int)(playerAbsolutePosition.y * adjustedScale);
    
    // Adjust the player's Y position on the minimap to lower it
    playerYOnMap += 100; // Adjust the value as needed
    
    // Update the player's position on the minimap
    minimap->playerPosition.y = playerYOnMap;
/////////
    // Calculate the player's position on the minimap using the adjusted scaling factor
    int playerXOnMap = (int)(playerAbsolutePosition.x * adjustedScale);
...
void afficherTempsLevel(int tempsDebut, int dureeTotale, SDL_Surface *screen, TTF_Font *font) {
    ...
    int textPosX = screen->w - textSurface->w - 10; // 10 pixels from the right edge
    int textPosY = 10; // 10 pixels from the top edge
    
    SDL_Rect textRect = { textPosX, textPosY, 0, 0 }; // Position to render the text
